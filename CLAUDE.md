...
ファイルを新規作成・編集した後は必ず以下の形式で説明すること。
コードの羅列だけで終わらせない。「何を書いたか」より「なぜそう書いたか」を重視する。

### 新規作成の場合
1. **このファイルの役割** : プロジェクト全体の中でこのファイルが担う責務を1〜2文で説明
2. **主要な定義・クラス・関数** : 何が定義されているか、それぞれ何をするか
3. **他ファイルとの関係** : どのファイルからimportされるか、逆に何をimportするか
4. **実装上の注意点** : ハマりやすいポイントや設計上の意図があれば補足

### 編集の場合
1. **変更箇所** : どこを・何のために変更したか
2. **変更の意味** : なぜその変更が必要だったか
3. **影響範囲** : この変更によって他ファイルに影響が出る可能性があるか


## ✋ チェックポイントのルール（省略禁止）

### 自動検証（Claude Codeがコマンドを実行して結果を報告）
以下のタイミングで必ずコマンドを実行し、結果をそのまま貼ること。
エラーがあれば次のステップに進まず、その場で解決する。

| タイミング | コマンド |
|---|---|
| pubspec.yaml変更後 | `flutter pub get` |
| モデルファイル作成後 | `flutter pub run build_runner build` |
| Dartファイル新規作成後 | `flutter analyze lib/<作成したファイル>` |
| 各画面実装完了後 | `flutter analyze lib/` |

### 人間確認（必ず作業を止めて確認を求める）
以下のタイミングで **必ず作業を止め** 、確認を求めること。
「確認できました」の返答があるまで次のステップに進まない。
| タイミング | 確認内容 |
|---|---|
| 2-8 BottomTab実装後 | シミュレーターで起動・4タブが表示されるか |
| 3-1 SplashScreen完了後 | 起動→HomeScreen遷移・コンソールにエラーなし |
| 3-2 HomeScreen完了後 | StreakHeroCard・ボタン・統計カードが表示されるか |
| 3-3 QuizScreen完了後 | 問題取得・選択肢タップ・回答ボタンが動くか |
| 3-4 ExplainScreen完了後 | 正誤表示・GoalModal・Hive保存が動くか |
| 各画面完了後 | `quizt_mockup.html` と見比べてUIがOKか |

### 絶対に飛ばさないルール
- 人間確認の待ちをスキップして先に進まない
- 「たぶん動く」でPROGRESS.mdを `[x]` にしない
- エラーログは省略せず全文貼る
- 作業完了時は必ず「PROGRESS.mdとDEVLOG.mdを更新しました」と報告する
